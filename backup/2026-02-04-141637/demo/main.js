...
async function bakeFractalAsset() {
  const seed = Number(seedInput?.value ?? 1337);
  const radius = Number(radiusInput?.value ?? 6);
  const extent = radius * 2.6;
  const steps = Math.round(clamp(radius * 16, 36, 180));
  setStatus("Baking fractal asset...");
  const asset =
    state.fractalAsset && assetMatches(state.fractalAsset, { seed, extent, gridSize: steps })
      ? state.fractalAsset
      : await runFractalPrepass({ seed, extent, steps, heightScale: 2.6 });
  state.fractalAsset = asset;
  saveFractalAssetToStorage(asset);
  downloadFractalAsset(asset);
  setStatus("Fractal asset baked and downloaded.");
}
...
function buildGeometry(layer, size = 1, heightScale = 1.2, seed = 1337) {
  ...
  const addHexSurface = (cell, center, height, color, material) => {
    ...
    const centerNoise = isWater
      ? 0
      : (hash01(cell.q * 19.13 + cell.r * 27.91 + seed * 0.13) - 0.5) *
        heightScale *
        0.04;
  ...
  layer.cells.forEach((cell, index) => {
    ...
    if (terrain.feature === MicroFeature.Tree) {
      const seedValue = (Math.sin(cell.q * 12.989 + cell.r * 78.233) * 43758.5453) % 1;
...
function surfaceFromField(sample, slope) {
  if (sample.water > 0.55 || sample.height < 0.18) {
    return SurfaceCover.Water;
  }
  if (slope > 0.65 || sample.rockiness > 0.7) {
...
function buildFractalGeometry(options) {
  const { seed, extent, steps, heightScale, prepass } = options;
  ...
  const { vertices, boxMin, boxMax, addTriangle, addTreeMesh, addBounds } = builder;
  ...
  for (let i = 0; i < gridSize; i += 1) {
    ...
    for (let j = 0; j < gridSize; j += 1) {
      ...
      const surface = surfaceFromField(avgSample, slope);
      const color = colorForSurface(surface);
      const material = materialForSurface(surface);
      ...
      if (surface === SurfaceCover.Grass && avgSample.moisture > 0.55 && slope < 0.35) {
        const centerX = (x0 + x1) * 0.5;
        const centerZ = (z0 + z1) * 0.5;
        const treeSeed = hash01((i + 1) * 37.9 + (j + 1) * 71.3 + seed * 0.17);
        if (treeSeed > 0.72 && avgSample.water < 0.2) {
          const treeHeight = (h00 + h10 + h01 + h11) * 0.25;
          addTreeMesh([centerX, 0, centerZ], treeHeight, treeSeed);
        }
      }
    }
  }
...
async function rebuildScene() {
  ...
  if (mode === "fractal") {
    ...
    if (!assetMatches(prepass, { seed, extent, gridSize: steps })) {
    ...
    statusPrefix = `Fractal ${steps}x${steps}`;
  } else {
    const layer = generateTemperateMixedForest({ seed, radius });
    geometry = buildGeometry(layer, 1, 1.2, seed);
    state.orthoScale = clamp(radius * 1.8, 6, 30);
    state.radius = clamp(radius * 2.4, 10, 40);
    statusPrefix = `Hex Cells: ${layer.cells.length}`;
  }
