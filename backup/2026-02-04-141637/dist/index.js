const __PLASIUS_IMPORT_META_URL__ = import.meta.url;
...
function selectSurface(height, moisture, noise) {
  if (height < 0.12) {
...
function selectFeature(surface, height, moisture, noise) {
...
function generateTemperateMixedForest(options) {
  ...
  const terrain = cells.map((cell) => {
    const base = hashCell(cell, seed, 4660);
    const heatNoise = hashCell(cell, seed, 21317);
    const moistureNoise = hashCell(cell, seed, 40092);
    const featureNoise = hashCell(cell, seed, 30635);
